    ...
    <div className='h-full relative overflow-scroll backdrop-blur-lg'>
          ...
          <div key={index} className={`flex items-end justify-end gap-2 ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
            {
              msg.image ? (
                ...
                <p className={`p-2 max-w-50 md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500 text-white ${msg.senderId !== '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
              )}
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center px-3 rounded-full bg-gray-300'>
          <input type="text" placeholder='Message' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-gray-900 placeholder-gray-700' />
